...
def search(request):
    if request.method == 'POST':
        ...
        if term and term != "": #if a search term exists and it is not an empty string:
            ...
            else: #there are no matches - #d redirect to the home page and isplay an alert 
...
def create_entry(request):
    ...
    if request.method == 'POST':
        ...
        if not title or not content:
            ...
            messages.success(request, 'Entry saved successfully') #this is a better way to pass messages - standard for Django
